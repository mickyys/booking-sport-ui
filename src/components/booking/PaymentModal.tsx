...
interface PaymentModalProps {
    ...
    court: Court;
    onClose: () => void;
    onConfirm: (method: 'fintoc', guestDetails?: GuestDetails) => void;
...
export const PaymentModal: React.FC<PaymentModalProps> = ({ 
  ...
  const handlePayment = (method: 'fintoc') => {
    if (!validate()) return;
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
             <p className="text-sm font-medium text-slate-700 mb-2">Selecciona medio de pago:</p>
             <button
          ...
          <div className="mt-6 flex items-start gap-2 text-xs text-slate-400 bg-slate-50 p-3 rounded-lg">
             ...
             <p>Serás redirigido a la pasarela de pago segura. No almacenamos tus datos bancarios.</p>
          </div>
        </div>
      </motion.div>
    </div>
